    ...
    <div className="relative min-h-screen pb-20 overflow-hidden bg-gray-50 dark:bg-gray-950">
      ...
      <div className="max-w-7xl mx-auto px-8 mt-12">
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'payments' && <PaymentsTab key="payments" orders={orders} refresh={fetchData} />}
              ...
              <div className="flex flex-wrap gap-8 items-end">
                <div className="space-y-3">
                   ...
                   <select value={order.status} onChange={(e) => updateStatus(order.id, e.target.value)} className="block w-full bg-gray-100 dark:bg-gray-800 rounded-2xl p-5 text-sm font-bold outline-none border-none focus:ring-2 focus:ring-purple-500 appearance-none min-w-[200px]">
                     <option value="pending" className="bg-white dark:bg-gray-800">🟡 Pending Analysis</option>
                     <option value="in-progress" className="bg-white dark:bg-gray-800">🔵 Expert Working</option>
                     <option value="completed" className="bg-white dark:bg-gray-800">🟢 Quality Approved</option>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
      {pending.map(order => (
        <div key={order.id} className="bg-white dark:bg-gray-900 p-10 rounded-[3.5rem] shadow-2xl border border-gray-100 dark:border-gray-800 relative overflow-hidden group">
          ...
          <button onClick={async () => { await supabase.from('orders').update({ payment_status: 'paid', status: 'in-progress' }).eq('id', order.id); refresh(); }} className="w-full py-6 bg-green-600 text-white rounded-[1.5rem] font-black text-xs uppercase tracking-[0.3em] shadow-xl shadow-green-600/20 hover:scale-[1.02] transition-all">
             Verify & Unlock
          </button>
        </div>
      ))}
